...
def V(x):
    return np.sum((x-a[None,:])**2, axis=1)/2 - np.log(1+np.exp(-2*np.einsum('ij,j->i',x, a)))

def dV(x):
    return x - a[None,:] + 2*a[None,:]/(1+np.exp(2*np.einsum('ij,j->i',x, a)[:,None]))

def V_plot(x):
    return np.exp(-np.sum((x-a[None,:])**2, axis=1)/2 + np.log(1+np.exp(-2*np.einsum('ij,j->i',x, a))))
...
if EMPIRICAL_ON:
    ...
    if SGD_ON:
        ...
        plotter = utils.Plotter2D(os.path.join(figs_path, experiment_name, 'SGD'), V_plot, np.linspace(-4,4,51),np.linspace(-4,4,51))
    ...
    if MALA_ON:
        ...
        plotter = utils.Plotter2D(os.path.join(figs_path, experiment_name, 'MALA'), V_plot, np.linspace(-4,4,51),np.linspace(-4,4,51))
    ...
    if WP_ON:
        ...
        for ctr, T in enumerate(WP_T_arr):
            ...
            plotter = utils.Plotter2D(os.path.join(figs_path, experiment_name, 'WProx'+str(T)), V_plot, np.linspace(-4,4,51),np.linspace(-4,4,51))
           
            X_KER2 = X_inits
            plotter.do_plotting(X_KER2, name = "0", title="BRWP iter 0")  
            for i in tqdm(range(1, len(steps_arr))):
                X_KER2 = wass_opt_lib.update_once(X_KER2,V,dV,beta,T,stepsize)
                energybar_KER2[ctr,i] = compute_KL(X_KER2)
                if i%50 == 0:
                    plotter.do_plotting(X_KER2, name = str(i), title="BRWP iter "+str(i))   
            # if np.var(X_KER2) < 0:
            #     raise Exception("??")
