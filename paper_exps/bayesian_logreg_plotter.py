...
figs_path = os.path.join('figs', "logreg", str(problem_dim)+'_'+str(logistic_samples)+'_'+str(alpha),"beta"+str(beta)+"ss"+str(stepsize))
...
ax_err_WP.plot(steps_arr, errbar_sgd, label="SGD")
ax_err_WP.plot(steps_arr, errbar_MALA, label="MALA")
for ctr, WP_T in enumerate(WP_T_arr):
    if WP_T == 0.001: #or WP_T == 0.1:
        continue
    ax_err_WP.plot(steps_arr, errbar_KER2[ctr,:], label="WProx Kernel T="+str(WP_T))

ax_err_WP.set_xlabel("Iteration")
ax_err_WP.set_ylabel("Error")
ax_err_WP.legend()
...
fig_err_WP, ax_err_WP = plt.subplots()
# fig_err_WP.suptitle("Error")
ax_err_WP.plot(steps_arr, err2bar_sgd, label="SGD")
ax_err_WP.plot(steps_arr, err2bar_MALA, label="MALA")
for ctr, WP_T in enumerate(WP_T_arr):
    if WP_T == 0.001: #or WP_T == 0.1:
        continue
    ax_err_WP.plot(steps_arr, err2bar_KER2[ctr,:], label="WProx Kernel T="+str(WP_T))
