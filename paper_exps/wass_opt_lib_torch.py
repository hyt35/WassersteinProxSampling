...
def sample_V(X_list, V, sigma, n_iter = 100):
    ...
    for i in range(n_iter):
        X_list_perturbed = X_list + sigma * torch.randn_like(X_list)
...
def compute_score(W_list, V, dV, beta, T, sample_iters = 25, compute_energy = False, **kwargs):
    ...
    if compute_energy: # lazy bool
        ...
        perturbed_X = (W_list[None,:,:]).repeat(sample_iters, 1,1) + np.sqrt(2*beta*T) * torch.randn(sample_iters, *W_list.shape).to(device)
        ...
        mat_differences_perturbed = - (perturbed_X[None,:,:,:]).repeat(num_N,1,1,1)\
                    + (W_list[:,None,None,:]).repeat(1,sample_iters,1,1)
                    
        # ^ of shape [N,sample_iters, N,d]. mat_differences[p,q,...] = w_p - x_q (with additional outer sum w_i information)


        squared_differences_perturbed = torch.sum(mat_differences_perturbed**2, dim=3) # of shape [N,sample_iters,N]
        perturbed_X = perturbed_X.reshape((-1,perturbed_shape[-1]))
        ...
        unscaled_density = torch.exp(-1/(2*beta) * (perturbed_V[None,:] + squared_differences_perturbed/(2*T))) # [N,sample_iters,N]
        scaled_density = unscaled_density/normalizing_constants[:,None,None] # [N, sample_iters, N]
        
        rho_perturbed = torch.sum(scaled_density, dim=0) # sum over N to get log rho_T(x)
        log_rho = torch.log(rho_perturbed) # [sample_iters, N]
        ...
        term_1 = torch.exp(-perturbed_V/(2*beta))
        term_2 = perturbed_V + log_rho*beta
        # expectation over x
        numerator = torch.mean(term_1*term_2, dim=0) # [N]

        # denominator is simply the normalizing constant
        denominator = normalizing_constants

        energy = torch.sum(numerator/denominator) # technically should be mean but this would require changing other things
