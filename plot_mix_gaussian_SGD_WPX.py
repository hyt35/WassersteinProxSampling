...
def V(x):
    return np.sum((x-a[None,:])**2, axis=1)/2 - np.log(1+np.exp(-2*np.inner(x, a).ravel()))

def dV(x):
    return x - a[None,:] + 2*a[None,:]/(1+np.exp(2*np.inner(x, a[None,:])))
...
if EMPIRICAL_ON:
    ...
    if SGD_ON:
        ...
        plotter = utils.Plotter2D(os.path.join(figs_path, experiment_name, 'SGD'), V, np.linspace(-4,4,51),np.linspace(-4,4,51))
    ...
    if MALA_ON:
        ...
        plotter = utils.Plotter2D(os.path.join(figs_path, experiment_name, 'MALA'), V, np.linspace(-4,4,51),np.linspace(-4,4,51))
    ...
    if WP_ON:
        ...
        for ctr, T in enumerate(WP_T_arr):
            ...
            plotter = utils.Plotter2D(os.path.join(figs_path, experiment_name, 'WProx'+str(T)), V, np.linspace(-4,4,51),np.linspace(-4,4,51))
