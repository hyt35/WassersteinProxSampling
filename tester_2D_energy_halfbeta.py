...
if TEST_FN == "double_banana":
    ...
    figs_path_base = os.path.join("figs", "tester2D_energy_halfbeta", TEST_FN)
...
energy_arr = np.zeros(n_iters+25000)
...
energy_list = np.zeros(n_iters+25000) # why did i change the name here?

X_list = initer() # gaussian
figs_path = os.path.join(figs_path_base, "SGD", "beta"+str(beta)+"T_"+str(T)+"_ss"+str(stepsize),'dat')
if TEST_FN == "double_banana" or TEST_FN == "minus_double_banana":
    plotter = utils.Plotter2D(figs_path, V, np.linspace(-4,4,51),np.linspace(-4,4,51))
